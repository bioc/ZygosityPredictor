 ...
 #print(region_to_load)
  gr_list <- lapply(vcf_in, function(VCF){
    ...
    if(is(VCF, "TabixFile")){
      ...
      if(str_detect(VCF, paste0("chr", unlist(str_replace(chrom, "chr", "")), "[^0-9]"))){
        ##prind(1"noTabix_det")
        ##prind("file detected")
...
#   func_end()
# }
# prioritize_combination <- function(main_muts, snp_ids){
#   #print(1)
#   unrel_rows <- rownames(mat_phased) %>% .[which(!. %in% c(main_muts, snp_ids))]
#   mat_tmp <- mat_dist
...
#   return(next_comb)
# }
# prioritize_combination_new <- function(main_muts, snp_ids){
#   #print(1)
#   unrel_rows <- rownames(mat_phased) %>% .[which(!. %in% c(main_muts, snp_ids))]
#   mat_tmp <- mat_dist
#   mat_tmp[unrel_rows,] <- NA
...
#   shortest <- unphased[which(relevant_dists==min(relevant_dists, na.rm=T))] 
#   
#   ## check if any main mut is already phased to a snp:
#   
#   ## pick first one, if two have the same distance
#   #print(shortest)
#   #print(mat_phased)
...
eval_phasing_new <- function(all_comb, df_gene, printLog, verbose){
  func_start()
#  req_output_cols <- c("gene", "n_mut", "score", "conf",  
 #                      "general_info")
  ...
  annotated_result <- wt_cp_result %>%
    mutate(conf=conf, 
           ...
           # eval_by=case_when(
           #   score==0 ~ NA,
           #   phasing=="exclusion" ~ "aff_cp",
           #   TRUE ~ paste(phasing,"phasing", sep="-")
           # )
           )
